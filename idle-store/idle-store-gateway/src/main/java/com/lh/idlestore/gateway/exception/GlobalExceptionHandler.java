package com.lh.idlestore.gateway.exception;

import cn.dev33.satoken.exception.NotLoginException;
import cn.dev33.satoken.exception.NotPermissionException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.lh.framework.common.response.Response;
import com.lh.idlestore.gateway.enums.GatewayResponseCodeEnum;
...
public class GlobalExceptionHandler implements ErrorWebExceptionHandler {
    ...
    public Mono<Void> handle(ServerWebExchange exchange, Throwable ex) {
        ...
        if (ex instanceof NotLoginException) { // 未登录异常
            ...
            // 构建响应结果
            result = Response.fail(GatewayResponseCodeEnum.UNAUTHORIZED.getErrorCode(), ex.getMessage());
        } else if (ex instanceof NotPermissionException) { // 无权限异常
            response.setStatusCode(HttpStatus.FORBIDDEN);
            // 构建响应结果
            result = Response.fail(GatewayResponseCodeEnum.FORBIDDEN);
        } else { // 其他异常，则统一提示 “系统繁忙” 错误
            response.setStatusCode(HttpStatus.INTERNAL_SERVER_ERROR);
            result = Response.fail(GatewayResponseCodeEnum.SYSTEM_ERROR);
        }
